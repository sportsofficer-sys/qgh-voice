...
test('voice listener cancels pending continuous restarts and caches local availability', async () => {
  ...
  const environment = createWorkspaceEnvironment(documentRef, {
    SpeechRecognition: LocalRecognition,
    setTimeout(callback) {
  ...
  environment.workspace.runCommand(command('set-listening-mode', { mode: 'continuous' }));
  recognition.onend();
  assert.equal(timers.size, 1);
  const [restartId, restart] = timers.entries().next().value;
  timers.delete(restartId);
  restart();
  await tick();
  await tick();
  assert.equal(recognition.startCount, 3);

  recognition.onend();
  assert.equal(timers.size, 1);
  environment.rootListeners.get('blur')();
  assert.equal(timers.size, 0);
  assert.equal(recognition.startCount, 3);
});

test('voice listener cancels a delayed press-to-talk start before it becomes active', async () => {
  const instances = [];
  ...
  const documentRef = new FakeDocument();
  createWorkspaceEnvironment(documentRef, { SpeechRecognition: DelayedRecognition });
  const mic = documentRef.querySelector('.voice-mic');
  mic.dispatchEvent(new FakeEvent('pointerdown', { pointerId: 1 }));
  await tick();
  await tick();
...
test('voice listener honours a rapid press-to-talk re-press after the prior session ends', async () => {
  const instances = [];
  ...
  const documentRef = new FakeDocument();
  createWorkspaceEnvironment(documentRef, { SpeechRecognition: ReleaseRecognition });
  const mic = documentRef.querySelector('.voice-mic');
  mic.dispatchEvent(new FakeEvent('pointerdown', { pointerId: 1 }));
  await tick();
  await tick();
...
test('voice listener stops continuous retries after microphone denial', async () => {
  const timers = new Map();
  let nextTimerId = 1;
  const instances = [];
  ...
  const mic = documentRef.querySelector('.voice-mic');
  const continuous = documentRef.querySelector('.voice-continuous').children[0];
  assert.equal(timers.size, 0);
  assert.equal(continuous.checked, false);
  assert.equal(mic.textContent, 'HOLD TO TALK');
});

test('voice listener rechecks an externally downloading local pack', async () => {
  const timers = new Map();
  let nextTimerId = 1;
  let availability = 'downloading';

  class DownloadingRecognition {
  ...
  const documentRef = new FakeDocument();
  createWorkspaceEnvironment(documentRef, {
    SpeechRecognition: DownloadingRecognition,
    setTimeout(callback) {
      const id = nextTimerId;
      nextTimerId += 1;
      timers.set(id, callback);
  ...
  const [checkId, check] = timers.entries().next().value;
...
test('voice preparation coalesces duplicate local-pack installation requests', async () => {
  let installCalls = 0;
  let completeInstall;

  class DownloadableRecognition {
    constructor() {
      this.processLocally = false;
    }
    ...
    static install() {
      installCalls += 1;
      return new Promise(resolve => { completeInstall = resolve; });
    }
  }

  const documentRef = new FakeDocument();
  createWorkspaceEnvironment(documentRef, { SpeechRecognition: DownloadableRecognition });
  const settings = documentRef.querySelector('.voice-settings-toggle');
  const prepare = documentRef.querySelector('.voice-prepare');
  settings.click();
  await tick();
  await tick();
