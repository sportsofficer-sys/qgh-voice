...
- **Press to Talk** is the default. Hold the microphone button, say one complete command, then release it.
- **Continuous Listening** is optional. Turn it on only in a quiet environment. A clear live microphone state remains visible while it is active.
...
For fully offline voice control, the device must provide its local English speech-recognition pack. Windows uses the browser or application speech capability available on that device. On Android, QGH Simulator uses Android's on-device recogniser on Android 12 or later when the installed device supports it. It never falls back to cloud recognition. If local recognition is unavailable, use the normal controls; simulator operation remains offline and unchanged.
...
- If the microphone indicator says that local recognition is unavailable, use the standard controls. No flight state is lost. On Android, install or update the device's offline English (India) voice-recognition pack if the phone offers one, then reopen the app.

## Offline use

The Windows and Android applications contain the simulator locally. The web edition works offline after its first successful load. Voice recognition also requires a compatible local device language pack; no controller voice recording is sent to a QGH Simulator server or to a cloud speech service.
